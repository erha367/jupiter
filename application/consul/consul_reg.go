...
func RegitserService() {
	...
	reg := &api.AgentServiceRegistration{
		ID:      fmt.Sprintf("%v-%v-%v", config.App.Name, config.App.IP, config.App.RpcPort), // 服务节点的名称
		...
		Check: &api.AgentServiceCheck{ // 健康检查
			...
			GRPC:                           fmt.Sprintf("%v:%v/%v", config.App.IP, config.App.RpcPort, config.App.Name), // grpc 支持，执行健康检查的地址，service 会传到 Health.Check 函数中
